...
router.get('/teacher-results', async (req, res) => {
    try {
        ...
        // Se verifica si el docente tiene autoevaluacion y evaluacion por parte de estudiantes
...
router.get('/teahcer-self-check', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error al verificar la autoevaluacion', error: error.message })
    }
})
...
router.post('/submit', async (req, res) => {
    try {
        ...
        res.status(201).json({ message: 'Evaluacion enviada correctamente', id: evaluation._id })
    } catch (error) {
        res.status(500).json({ message: 'Error al enviar la evaluacion', error: error.message })
    }
})
